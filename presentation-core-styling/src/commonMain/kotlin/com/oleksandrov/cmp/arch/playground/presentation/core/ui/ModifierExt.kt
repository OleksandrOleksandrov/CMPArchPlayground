package com.oleksandrov.cmp.arch.playground.presentation.core.ui
...
import androidx.compose.ui.unit.dp
import com.oleksandrov.cmp.arch.playground.presentation.core.styling.platform
...
fun Modifier.privacySensitive(effect: PrivacyEffect = PrivacyEffect.Redact()): Modifier {
    ...
    val isInRecentApps = rememberUpdatedState(!windowInfo.isWindowFocused)
    return if (isInRecentApps.value) {
        when (effect) {
...
fun Modifier.applyBlur(blurRadius: Dp = 15.dp): Modifier = when {
    // Android 12+ (API 31+)
    platform().isPlatform31Plus -> this.blur(blurRadius)
